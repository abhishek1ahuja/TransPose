...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, nw_cfg=None):
        ...
        if nw_cfg == None:
            ...
            self.conv1 = nn.Conv2d(inplanes, nw_cfg[0], kernel_size=1, bias=False)
            self.bn1 = nn.BatchNorm2d(nw_cfg[0], momentum=BN_MOMENTUM)
            self.conv2 = nn.Conv2d(nw_cfg[0], nw_cfg[1], kernel_size=3, stride=stride,
                                   padding=1, bias=False)
            self.bn2 = nn.BatchNorm2d(nw_cfg[1], momentum=BN_MOMENTUM)
            self.conv3 = nn.Conv2d(nw_cfg[1], planes * self.expansion, kernel_size=1,
                                   bias=False)
            self.bn3 = nn.BatchNorm2d(planes * self.expansion,
                                      momentum=BN_MOMENTUM)
        self.relu = nn.ReLU(inplace=True)
...
class TransPoseR(nn.Module):
    ...
    def __init__(self, block, layers, cfg, nw_cfg=None, **kwargs):
        self.inplanes = 64
        ...
        if nw_cfg == None:
            ...
            self.layer1 = self._make_layer(block, 64, layers[0], nw_cfg=nw_cfg[:6])
            self.layer2 = self._make_layer(block, 128, layers[1], nw_cfg=nw_cfg[6:14], stride=2)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, nw_cfg=None):
        downsample = None
        ...
        if nw_cfg == None:
            ...
            layers.append(block(self.inplanes, planes, stride, downsample, nw_cfg[0:2])) #call to downsample layer
            self.inplanes = planes * block.expansion
            for i in range(1, blocks):
                layers.append(block(self.inplanes, planes, nw_cfg=nw_cfg[i * 2: (i + 1) * 2]))
